...
export type ImgVideoBannerProps = {
  className?: string;
  video?: string;
  img?: string;
...
const ImgVideoBanner: React.FC<ImgVideoBannerProps> = ({ className, video, img, description }) => {
  ...
  return !isVisible ? (
    <div className={className || 'flex items-center justify-evenly'}>
      {video && (
        <div className='relative w-[120px] h-[100px] md:w-[140px] md:h-[120px] cursor-pointer' onClick={() => setIsVisible(true)}>
      ...
      <div className='w-[38%] text-[14px] leading-[25px] text-[#000000] dark:text-[#EDEDF9] transition-colors duration-1000'>{description}</div>
    </div>
  ) : (
